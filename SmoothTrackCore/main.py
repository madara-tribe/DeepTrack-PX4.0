...
def get_parser():
    ...
    parser.add_argument(
        "--yolo_onnx_path", "-yp", type=str, default="weights/yolov7Tiny_640_640.onnx"
    )
...
class SmoothTrack:
    ...
    def bbox_diff(self, box, label):
        ...
        diff_x, diff_y = (
            (
                center_x - self.prev_centers[label][0],
                center_y - self.prev_centers[label][1],
            )
            if label in self.prev_centers
            else (0, 0)
        )
    ...
    def track(self):
        ...
        try:
            while True:
                ...
                outputs, ori_images, ratio, dwdh, conf_thres = detector.inference_(
                    frame
                )
                ...
                for det in outputs:
                    ...
                    cv2.rectangle(
                        image, (moved[0], moved[1]), (moved[2], moved[3]), color, 2
                    )
                    cv2.putText(
                        image,
                        label,
                        (moved[0], max(0, moved[1] - 10)),
                ...
                if not self.single_mode and len(boxes) > 1:
                    ...
                    group_box = self.clip_box(
                        min(x1s), min(y1s), max(x2s), max(y2s), img_w, img_h
                    )
                    cv2.rectangle(
                        image,
                        (group_box[0], group_box[1]),
                    ...
                    cv2.putText(
                        image,
                        "Group",
                        (group_box[0], max(0, group_box[1] - 10)),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.6,
                        (255, 255, 0),
                        2,
