...
MidasInference::~MidasInference() { delete session; }
...
cv::Mat MidasInference::runInference(cv::Mat &img, std::string pkg_path) {
  ...
  Ort::Value inputTensor = Ort::Value::CreateTensor<float>(
      memory_info, input.data(), input.size(), inputNodeDims.data(), inputNodeDims.size());
  // create output Tenspr
  const std::vector<int64_t> output_shapes{1, H, W};
  Ort::Value output_tensor = Ort::Value::CreateTensor<float>(
      memory_info, results.data(), results.size(), output_shapes.data(), output_shapes.size());
  // Inference
  session->Run(run_options, &input_node_name, &inputTensor, 1U, &output_node_name, &output_tensor,
               1U);
