...
OnnxInferenceNode::OnnxInferenceNode(const rclcpp::NodeOptions &options = rclcpp::NodeOptions())
    : Node("px2", options) {
  publisher_ = this->create_publisher<custom_msgs::msg::AbsResult>(
      "inference", rclcpp::QoS{10}.transient_local());
  timer_ = this->create_wall_timer(500ms, std::bind(&OnnxInferenceNode::callbackInference, this));
...
void OnnxInferenceNode::callbackInference() {
  ...
  RCLCPP_INFO(rclcpp::get_logger("ImageSubscriber"), "Image received: %dx%d", latest_image_.cols,
              latest_image_.rows);
  ...
  std::cout << "Midas and YOLO Prepare" << yolo_image.channels() << "w" << yolo_image.cols << "h"
            << yolo_image.rows << std::endl;
...
std::list<float> OnnxInferenceNode::SearchMedian(std::vector<Result> resultVector,
                                                 cv::Mat &depth_resize, cv::Mat &yolo_result) {
  ...
  for (auto result : resultVector) {
    ...
    if (x >= w || y >= h) {
      RCLCPP_WARN(this->get_logger(), "Invalid ROI: x >= w or y >= h (x=%d, w=%d, y=%d, h=%d)", x,
                  w, y, h);
      continue;
    }
    ...
    cv::putText(yolo_result, std::to_string(x_real_coordinate), cv::Point(x, y + 50),
...
void OnnxInferenceNode::publishState(const custom_msgs::msg::AbsResult &message) {
  RCLCPP_INFO(this->get_logger(),
              "Publishing absolute_dist: %.2f, x_real_coordinate: %.2f, max_x: %.2f",
