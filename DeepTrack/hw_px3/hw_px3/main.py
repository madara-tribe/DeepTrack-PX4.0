...
class AbsSubscriber(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            AbsResult, "inference", self.listener_callback, 10
        )

        try:
            self.declare_parameter("arduino_path", "/dev/ttyACM1")
            arduino_path = (
                self.get_parameter("arduino_path").get_parameter_value().string_value
            )
            ...
            self.get_logger().info(
                f"Arduino initialized at {arduino_path}, angle set to {self.initial_value}"
            )
    ...
    def send_angle(self, angle):
        if self.ser and self.ser.is_open:
            try:
                angle_mapped = int(
                    180 - float(angle)
                )  # Mapeo final: ajuste para tu servo
                self.ser.write(f"{angle_mapped}\n".encode())
                self.get_logger().info(
                    f"Sent angle: {angle_mapped} (Original: {angle})"
                )
    ...
    def listener_callback(self, msg):
        ...
        if angle_deg is None:
            self.get_logger().warn(
                f"x={msg.x_real_coordinate} out of range, angle not sent."
            )
            return
